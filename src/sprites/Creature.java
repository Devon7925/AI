...
public class Creature extends Sprite {
    ...
    public Creature(double x, double y, double rot1, int range, Network net, Color c) {
        super(x, y, 10, 40, range, c);
        rot = rot1;
        this.net = net;
        female = rnd.nextBoolean();
    }
    ...
    void drawex(Graphics2D g2) {
        for (int i = 0; i < 5; i++) {// draw input lines
            g2.setColor(c);
            g2.setColor(new Color((inputs[9 * i] ? 128 : 0) + (inputs[9 * i + 1] ? 64 : 0),
                    (inputs[9 * i + 2] ? 128 : 0) + (inputs[9 * i + 3] ? 64 : 0),
                    (inputs[9 * i + 4] ? 128 : 0) + (inputs[9 * i + 5] ? 64 : 0)));
    ...
    @Override
    void update() {
        HashSet<Sprite> set = holder.query(new Square(new Point(loc.getX(), loc.getY()), 32 + 2 * getR()),
                new Square(new Point(0, 0), 2 * Settings.range));
        ...
        if (outputs[6]) {// eat & breed
            ...
            if (hunger <= 97) {
                Arith.filter(Food.class, set).stream()
                        .filter(n -> Calc.dist(n.getX() - getX(), n.getY() - getY()) < n.getR() + getR()).forEach(n -> {
                            hunger += 3;
                            n.incrementR(-1);
                        });
            }
                ...
                for (Creature cr : Arith.filter(Creature.class, set)) {
                    if (cr != this) {
                        if (cr.female != female && cr.consenting && Arith.dist(cr, this) < r + cr.r && !bred
                            ...
                            breed = new Creature(newcx, newcy, rnd.nextInt(6), range, Arith.mix(net, cr.net),
                                    Arith.mix(c, cr.c));
        ...
        if (outputs[7]) {// shoot
            if (!stopb) {
                holder.queue.add(
                        new Bullet(getX() + Math.cos(rot) * 30, getY() - Math.sin(rot) * 30, rot, c, Settings.range));
                hunger -= 0.25;
                stopb = true;
            }
        } else
